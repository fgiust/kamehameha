...
GA4 uses a **hybrid channel**: gtag.js (loaded from `googletagmanager.com/gtag/js` — this is the official GA4 library URL, not Google Tag Manager) when the script loads; first-party Measurement Protocol (`/api/analytics`) when the script is blocked. The choice is cached in `sessionStorage` for the tab — never both channels for the same session.
